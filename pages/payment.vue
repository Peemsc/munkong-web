          ...
          <div class="blog-content">
            <div class="blog-row align-items-center">
              <ul class="nav nav-blog">
                <li><a href="/">Home</a></li>
                <li class="active"><a href="#">{{ $t('paymentMethod') }}</a></li>
              </ul>
            ...
            <div class="blog-details text-center">
              <template v-if="setting">
                <div v-if="$i18n.locale === 'th'" class="details wow fadeIn" v-html="setting.payment"></div>
                <div v-if="$i18n.locale === 'en'" class="details wow fadeIn" v-html="setting.paymentEN"></div>
              </template>

              <div class="row">

                <div class="col-md-4 col-6 wow fadeInUp" v-for="(payment, index) in listPayment" :key="index">
                  <div class="card-icon payment">
                    <div class="icon">
                      <img :src="payment.cover">
                    </div>
              ...
              <template v-for="(payment, index) in listPayment">
                <client-only>
                  <div class="details wow fadeIn pt-3" :key="index">
                    <h3>{{ index+1 }}. {{ payment.paymentMethod }}</h3>

                    <p v-html="payment.desc"></p>

                    <a :href="payment.image" data-fancybox>
                      <img class="img-fluid mw-720 mb-3" :src="payment.image" alt="">
                    </a>
...
export default {
  ...
  async fetch () {
    await this.$store.dispatch('payment-method/showPriority')
    await this.$store.dispatch('setting/getSetting', 'payment')

    this.meta.title = this.$i18n.locale === 'th' ? 'วิธีชำระเงิน' : 'Payment';
    this.meta.hid = this.$i18n.locale === 'th' ? 'วิธีชำระเงิน' : 'Payment';
    this.meta.name = this.$i18n.locale === 'th' ? 'วิธีชำระเงิน' : 'Payment';
    this.meta.content = this.$i18n.locale === 'th' ? 'วิธีชำระเงิน' : 'Payment';
  ...
  methods: {
    shareFacebook() {
      window.open(
        "https://www.facebook.com/sharer.php?u=" +
          encodeURIComponent(
            `${window.location.origin}/payment?openExternalBrowser=1`
          )
      );
    },
    shareTwitter() {
      const url = `${window.location.origin}/payment?openExternalBrowser=1`;
    ...
    copyToClipboard() {
      const url = `${window.location.origin}/payment`;
  ...
  computed: {
    listPayment () {
      return this.$store.getters['payment-method/getPaymentMethod']
    },
    setting () {
